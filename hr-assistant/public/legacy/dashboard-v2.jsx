...
function DashboardOps({ t, openEmployee, openSurvey, setRoute }) {
  ...
  return (
    <div className="main" data-screen-label="02 Dashboard · Operativo" style={{ gap: 16 }}>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1.5fr 1fr", gap: 16 }}>
        <div className="card-flush">
          ...
          <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ marginLeft: 8, display: "inline-flex", border: "1px solid var(--line)", borderRadius: 8, padding: 2 }}>
              {[
                { k:"all", l: t("filter.all") },
                { k:"atrisk", l: t("label.atRisk") },
                { k:"watch", l: t("label.watch") }
              ].map(f => (
            ...
            <select value={filter.startsWith("dept-") ? filter : "all-dept"}
              onChange={e => setFilter(e.target.value === "all-dept" ? "all" : e.target.value)}
              style={{
                border: "1px solid var(--line)", borderRadius: 8, padding: "5px 10px",
                background: "var(--paper)", fontSize: 11.5, color: "var(--ink-2)", height: 28
              }}>
                  ...
                  <tr key={e.id} onClick={() => openEmployee(e)}>
                    ...
                    <td>
                      <div className="row" style={{ gap: 6 }}>
                        <div className="bar" style={{ width: 60 }}><i style={{
                          width: `${e.workload}%`,
                          background: e.workload > 80 ? "var(--rose)" : e.workload > 65 ? "var(--amber)" : "var(--olive)"
                        }}/></div>
          ...
          <div style={{ padding: "10px 16px", borderTop: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 10 }}>
            <span className="muted" style={{ fontSize: 12 }}>
              {t("lang")==="ES"?`Mostrando 14 de ${filtered.length}`:`Showing 14 of ${filtered.length}`}
            </span>
        ...
        <div className="col" style={{ gap: 16 }}>
          ...
          <div className="card">
            ...
            <div style={{ marginTop: 10 }}>
              <LineChart data={D.COMPANY.trend} height={120} />
            </div>
          </div>
          ...
          <div className="card-flush">
            ...
            <table className="tbl">
              <thead>
                <tr>
                  <th>{t("table.dept")}</th>
                  <th style={{ width: 60 }}>{t("lang")==="ES"?"#":"#"}</th>
                  <th>{t("table.sat")}</th>
      ...
      {/* Lower row: surveys + alerts + workload */}
      <div className="grid" style={{ gridTemplateColumns: "1.2fr 1fr 0.8fr", gap: 16 }}>
        <div className="card-flush">
          <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--line)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <button className="btn sm ghost" onClick={() => setRoute("surveys")}>{t("label.viewAll")}</button>
          </div>
          <table className="tbl">
            <thead>
              <tr>
